...
// ─── FAQs (reader-style Q&A pairs) ──────────────────────────────
// Generated by AKIRA's FAQ generator (core/faq_generator.py) so users
// can understand "qué es X?" without reading every source. The shape
// of each item is:
//   { question, answer, source_count }
// Cached at the edge for 60s + 24h SWR — same as contradictions.
...
export async function fetchFaqs(
  ...
  try {
    const res = await fetch(`${API_BASE}/api/clusters/${clusterId}/faqs`);
    if (!res.ok) return null;
    return (await res.json()) as FaqsResponse;
  } catch {
    return null;
  }
}
